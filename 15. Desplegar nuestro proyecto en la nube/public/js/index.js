...
socket.on('mensajeLogout', mensajeLogout);

async function mensajeLogout(usuario) {
    Swal.fire({
        position: "center",
        icon: "success",
        title: `Hasta pronto ${usuario}!`,
...
botonEnviarProducto.addEventListener('click', e => {
    ...
    if (inputTitulo.value && inputPrecio.value && inputImagen.value) {
        const producto = {
